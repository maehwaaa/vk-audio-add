...
/**
 * Параметры плейлиста из URL текущей вкладки.
 *
 * Новый формат:
 *   https://vk.com/music/playlist/231820652_1141_0bcb1736a1cb8e41e7
 *
 * Старый формат:
 *   https://vk.com/audios231820652?z=audio_playlist231820652_1141
 */
function parsePlaylistFromUrl() {
  ...
  throw new Error(
    ...
    "Пример: https://vk.com/music/playlist/231820652_1141_0bcb1736a1cb8e41e7"
  )
}
...
/**
 * POST через внутренний ajax ВК (как кнопка «Добавить» на сайте).
 * Обычно не вызывает капчу так агрессивно, как api.vk.com/method/audio.add.
 */
...
async function addAudioToLibrary(song, accessToken, apiVersion) {
  // 1. Внутренний vk.api на странице (как кнопка «+»)
  try {
  ...
  // 2. api.vk.com с web-токеном (работало у вас ~49 треков до капчи)
  try {
  ...
  // 3. al_audio.php — только если есть hash или track_code
  return await addAudioViaAlAudio(song)
}

/**
 * audio.get иногда не отдаёт track_code — добираем через getById.
 */
...
async function waitForCaptchaSolve() {
  ...
  prompt(
    "После того как вручную добавили одну песню и прошли капчу,\n" +
    "нажмите OK, чтобы скрипт продолжил (или Отмена — остановить)."
  )
}

/**
 * Загружает все треки плейлиста через audio.get (не из DOM).
 */
...
async function main() {
  ...
  console.log("Загружаем полный список треков через API (не из DOM)...")
  ...
  console.log(`В плейлисте ${songs.length} треков (DOM показывает только подгруженные — это нормально)`)
  ...
  console.log("Метод: vk.api → api.vk.com → al_audio.php")
  console.log("Остановка: stopVkAdd = true")
